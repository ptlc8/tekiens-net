<template>
    <section id="faq">
        <article>
            <h2>FAQ</h2>
            ...
        </article>
    </section>
    <section>
        <article>
            <h2 id="contribute">Contribuer au projet <a href="https://tekiens.net">Tekiens.net</a> ?</h2>
            Vous êtes intéressés pour découvrir le code, ou vous voulez simplement participer à l'amélioration de la plateforme ?
            <br />
            Rejoignez nous en CY307 ou sur les internets !
            <br />
            Le code source est disponible sur <a href="https://gitlab.com/ptlc8/tekiens-net" target="_blank">GitLab</a>.
        </article>
    </section>
    <section>
        <article>
            <h2 id="history">Historique</h2>
            Tekiens.net est le site de la vie étudiante de CY Tech. (anciennement eistiens.net)
            <br />
            Le but de ce site est de centraliser les informations concernant les associations et les événements de l'école.
        </article>
    </section>
    <section>
        <article>
            <h2 id="problem">Un problème ?</h2>
            ...
        </article>
    </section>
</template>